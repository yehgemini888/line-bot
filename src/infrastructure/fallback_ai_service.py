...
class FallbackAIService:
    ...
    def _trip_circuit(self):
        ...
        logger.warning(f"⚡ [CircuitBreaker] Gemini 暫停 {CIRCUIT_BREAKER_COOLDOWN} 秒，直接使用 OpenAI")
    ...
    async def summarize(
        ...
        if self._is_circuit_open():
            logger.warning(f"⚡ [CircuitBreaker] Gemini 熔斷中，直接使用 OpenAI summarize")
        ...
        try:
            ...
            if result.success:
                return result
            logger.warning(f"⚠️ [Fallback] Gemini returned failure: {result.error_message}, switching to OpenAI")
            self._trip_circuit()
        except Exception as e:
            logger.warning(f"⚠️ [Fallback] Gemini raised exception: {e}, switching to OpenAI")
            self._trip_circuit()
    ...
    async def generate_simple(self, prompt: str) -> str:
        ...
        """
        if self._is_circuit_open():
            logger.warning(f"⚡ [CircuitBreaker] Gemini 熔斷中，直接使用 OpenAI generate_simple")
        ...
        try:
            ...
            if result:
                return result
            logger.warning(f"⚠️ [Fallback] Gemini generate_simple empty, switching to OpenAI")
            self._trip_circuit()
        except Exception as e:
            logger.warning(f"⚠️ [Fallback] Gemini generate_simple exception: {e}, switching to OpenAI")
            self._trip_circuit()
    ...
    async def analyze_image(
        ...
        """
        if self._is_circuit_open():
            logger.warning(f"⚡ [CircuitBreaker] Gemini 熔斷中，直接使用 OpenAI analyze_image")
        ...
        try:
            ...
            if result.success:
                return result
            logger.warning(f"⚠️ [Fallback] Gemini Vision returned failure: {result.error_message}, switching to OpenAI")
            self._trip_circuit()
        except Exception as e:
            logger.warning(f"⚠️ [Fallback] Gemini Vision raised exception: {e}, switching to OpenAI")
            self._trip_circuit()
    ...
    async def summarize_with_schema(
        ...
        """
        if self._is_circuit_open():
            logger.warning(f"⚡ [CircuitBreaker] Gemini 熔斷中，直接使用 OpenAI summarize_with_schema")
        ...
        try:
            ...
            logger.warning(f"⚠️ [Fallback] Gemini Structured Output 失敗: {e}, 切換到 OpenAI")
            self._trip_circuit()
